...
class Config(object):
    ...
    def __init__(self, params=None):
        params = params or {}
        self.db_config = params.get(u"db_config")
        self.ssm_service_config = params.get(u"ssm_service_config")
        self.watch_freq = params.get(u"WATCH_FREQ", self.DEFAULT_WATCH_FREQ)
        self.rotation_grace_period = params.get(u"ROTATION_GRACE_PERIOD")
        self.borrow_retry_count = params.get(
            u"BORROW_RETRY_COUNT", self.DEFAULT_BORROW_RETRY_COUNT
        )
        self.borrow_retry_interval_ms = params.get(
            u"BORROW_RETRY_INTERVAL_MS", self.DEFAULT_BORROW_RETRY_INTERVAL_MS
        )

    def validate(self):
        if self.db_config is None:
            return Error(u"db_config is required")
        err = self.db_config.validate()
        if err:
            return err
        if self.ssm_service_config is None:
            return Error(u"ssm_service_config is required")
        if not getattr(self.ssm_service_config, u"region", None):
            return Error(u"region is required")
        if self.watch_freq is None or self.watch_freq <= 0:
            return Error(u"WATCH_FREQ must be greater than 0")
        if self.rotation_grace_period is not None and self.rotation_grace_period <= 0:
            return Error(u"ROTATION_GRACE_PERIOD must be greater than 0")
        if self.borrow_retry_count <= 0:
            return Error(u"BORROW_RETRY_COUNT must be greater than 0")
        if self.borrow_retry_interval_ms < 0:
            return Error(u"BORROW_RETRY_INTERVAL_MS must be greater than or equal to 0")
        return None
...
class DbConfig(object):
    def __init__(self, params=None):
        params = params or {}
        self.secret_name = params.get(u"secret_name")
        self.ip_address = params.get(u"ip_address")
        self.port = params.get(u"port")
        self.db_name = params.get(u"db_name")
        self.param_str = params.get(u"param_str")
        self.pool_size = params.get(u"pool_size", 5)
        self.pool_name = params.get(u"pool_name", u"ssm_pool")

    def validate(self):
        if not self.secret_name:
            return Error(u"secret_name is required")
        if not self.ip_address:
            return Error(u"ip_address is required")
        if not self.port:
            return Error(u"port is required")
        if self.pool_size <= 0:
            return Error(u"pool_size must be greater than 0")
        return None
...
class DynamicSecretRotationDb(object):
    ...
    AUTH_ERROR_CODES = set([1044, 1045, 1698])
    UNSUPPORTED_PARAMS = set([u"loc", u"parseTime"])

    def __init__(self, params=None):
        params = params or {}
        self.config = params.get(u"config")
        self.db_conn = params.get(u"db_conn")
        ...
        for attempt in range(self.config.borrow_retry_count):
            try:
                ...
                if self.__is_authentication_error(exc):
                    logging.warning(u"authentication failed when borrowing connection, refreshing pool")
                    err = self.__refresh_pool(force=True)
                    if err:
                        logging.error(u"failed to refresh pool after authentication error: %s", err.message)
                        return None
                    with self._lock:
                ...
                logging.error(u"failed to get connection from pool: %s", unicode(exc))
                return None
    ...
    def init(self, config):
        ...
        self._watch_thread = threading.Thread(target=self.__watch_secret_change)
        self._watch_thread.setDaemon(True)
        self._watch_thread.start()
        logging.info(u"succeed to init db_conn")
        return None
    ...
    def __watch_secret_change(self):
        ...
        while not self._stop_event.wait(interval):
            ...
            if failures >= self.MAX_WATCH_FAILURES:
                ...
                logging.warning(u"applying exponential backoff, next retry in %d seconds", interval)
            else:
                # 恢复正常后，重置为原始间隔
                interval = self.config.watch_freq
    ...
    def __watch_change(self):
        ...
        if err:
            ...
            logging.error(
                u"failed to watch secret change (%d/%d): %s",
                self.watch_failures,
                self.MAX_WATCH_FAILURES,
                err.message,
            )
            return
    ...
    def __refresh_pool(self, force=False):
        ...
        with self._lock:
            current = self.db_conn
            if (not force) and (not self.closed) and current is not None and current.conn_key == conn_key:
                return None
        ...
        try:
            ...
            return Error(u"connect to cdb error: %s" % unicode(exc))
        ...
        with self._lock:
            if self.closed:
                ...
                return Error(u"dynamic secret rotation db is closed")
            old_cache = self.db_conn
            self.db_conn = cache

        if old_cache is not None and old_cache.user_name != account.user_name:
            logging.info(u"credential rotated: %s -> %s", old_cache.user_name, account.user_name)
    ...
    def __build_pool_config(self, account):
        ...
        extras = self.__parse_extra_params(db_config.param_str)
    ...
    def __parse_extra_params(self, param_str):
        ...
        for param in param_str.split(u"&"):
            if u"=" not in param:
                continue
    ...
    def __build_conn_key(self, account):
        raw = (u"{0}\0{1}".format(account.user_name, account.password)).encode("utf-8")
    ...
    def __is_authentication_error(self, exc):
        ...
        if errno in self.AUTH_ERROR_CODES:
            return True
        message = unicode(exc).lower()
        return u"access denied" in message or u"authentication" in message

    def __is_pool_exhausted(self, exc):
        return u"pool exhausted" in unicode(exc).lower()
